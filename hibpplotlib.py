...
def plot_geometry(ax, TF_coil_filename='TFCoil.dat',
                  camera_data_filename='port75.txt',
                  separatrix_data_filename='Globus_sep.txt',
    ...
    camera = np.loadtxt(camera_data_filename, skiprows=1)/1000
    for i in range(camera.shape[0]):
        ax.plot(camera[i, [3, 0]], camera[i, [4, 1]], color='tab:blue')
    ...
    if plot_sep:
        if separatrix_data_filename is not None:
            ...
            ax.plot(separatrix[:, 0], separatrix[:, 1], 'o', markersize=2,
                    color='b')  # 'tab:orange')
